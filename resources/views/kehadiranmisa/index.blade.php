...
<body>
    ...
    <main class="p-4 md:ml-64 h-auto pt-7 bg-white">
        ...
        <div class="text-center mb-4">
            <h2 class="text-2xl font-semibold leading-7 text-gray-900">Kehadiran Misa</h2>
            <p class="mt-2 text-sm leading-6 text-gray-600">Kehadiran-Kehadiran Misa kelompok pada misa</p>
            <p class="mt-2 text-sm leading-6 text-gray-600">
                Periode aktif: 
                @php
                    $periodeAktif = $periode->filter(function($p) {
                        return $p->status_periode == 1;
                    });
                @endphp
...
<section class="bg-white dark:bg-gray-900 p-3 sm:p-5 antialiased">
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                ...
                <div class="w-full md:w-1/2">
                    <form action="{{ route('kehadiranmisa.index') }}" method="GET" class="max-w-lg">
                        <div class="flex">
                            ...
                            <div class="relative w-full">
                                <input type="search" name="search" id="search-dropdown" 
                                       ...
                                       placeholder="Cari Kehadiran Misa Kelompok" />
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="w-full max-w-xs">
                            <select name="kelompokID" id="kelompokID" 
                                class="block w-full rounded-md border border-gray-300 py-1.5 px-3 text-gray-900 shadow-sm focus:border-indigo-600 focus:ring-2 focus:ring-indigo-600 sm:text-sm sm:leading-6 {{ $aktivPeriode ? '' : 'pointer-events-none opacity-50' }}">
                        ...
                        <form id="createForm" action="{{ route('kehadiranmisa.create') }}" method="GET" class="flex-none">
                            <input type="hidden" name="kelompokID" id="selectedKelompokID">
                            <button type="submit" class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800 {{ $aktivPeriode ? '' : 'pointer-events-none opacity-50' }}">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-4 border">Kelompok</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3 border">{{ $kehadiranmisa->jadwalMisa->kelompok->nama_kelompok }} ({{ $kehadiranmisa->jadwalMisa->periode->nama_periode }})</td>
                           ...
                           <!-- List Anggota yang Hadir -->
                            ...
                            <td class="px-4 py-3 border">
                                <ul>
                                    @foreach ($kehadiranmisa->kehadiranMisaAnggota as $kehadiran)
                                        @if ($kehadiran->status_kehadiran === 'ijin' && $kehadiran->keanggotaan && $kehadiran->keanggotaan->anggota)
                                            <li>- {{ $kehadiran->keanggotaan->anggota->nama }} ({{ $kehadiran->sebagai }})</li>
                                        @endif
                                    @endforeach
                                </ul>
                            ...
                            <td class="px-4 py-3 border">
                                <ul>
                                    @foreach ($kehadiranmisa->kehadiranMisaAnggota as $kehadiran)
                                        @if ($kehadiran->status_kehadiran === 'alpha' && $kehadiran->keanggotaan && $kehadiran->keanggotaan->anggota)
                                            <li>- {{ $kehadiran->keanggotaan->anggota->nama }} ({{ $kehadiran->sebagai }})</li>
                                        @endif
                                    @endforeach
                                </ul>
                            ...
                            <td class="px-4 py-3 flex items-center justify-center">
                                ...
                                <div id="{{ $kehadiranmisa->kehadiranMisaID }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm" aria-labelledby="apple-imac-27-dropdown-button">
                                        ...
                                        <li>
                                            <a href="/kehadiranmisa/{{ $kehadiranmisa->kehadiranMisaID }}" >
    ...
    <script>
        document.getElementById('kelompokID').addEventListener('change', function() {
            document.getElementById('selectedKelompokID').value = this.value;
        });
        
        document.getElementById('createForm').addEventListener('submit', function(e) {
            const kelompokID = document.getElementById('kelompokID').value;
            if (!kelompokID) {
                e.preventDefault();
                alert('Silakan pilih kelompok terlebih dahulu');
                return false;
            }
            document.getElementById('selectedKelompokID').value = kelompokID;
        });
