...
<body>
    ...
    <main class="p-4 md:ml-64 h-auto pt-7 bg-white">
        ...
        <div class="text-center mb-4">
            ...
            <p class="mt-2 text-sm leading-6 text-gray-600">Jadwal Misa Bertugas PAST</p>
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                <div class="w-full md:w-1/2">
                    ...
                    <form action="{{ route('jadwalmisa.index') }}" method="GET" class="max-w-lg">
                        <div class="flex">
                            ...
                            <div class="relative w-full">
                                <input type="search" name="search" id="search-dropdown" 
                                       ...
                                       placeholder="Cari Jadwal Misa (Jenis misa / Nama Kelompok)" />
                ...
                <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                    ...
                    <a href="/jadwalmisa/create" class="inline-block {{ $aktivPeriode ? '' : 'pointer-events-none opacity-50' }}">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-4 border">Kelompok yang ditugaskan</th>
                            <th scope="col" class="px-4 py-4 border">Anggota Bertugas</th>
                            <th scope="col" class="px-4 py-4 border">Tanggal Jam Misa</th>
                            <th scope="col" class="px-4 py-4 border">Tanggal dibuat</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3 border">{{ $jadwalmisa->jenismisa->jenis_misa }}</td>
                            <td class="px-4 py-3 border">{{ $jadwalmisa->kelompok->nama_kelompok }} ({{ $jadwalmisa->periode->nama_periode }})</td>
                            <td class="px-4 py-3 border">
                                <div class="flex flex-col">
                                    {{-- PiMi --}}
                                    @foreach ($jadwalmisa->kelompok->keanggotaans->where('periodeID', $jadwalmisa->periodeID) as $keanggotaan)
                                    @if ($keanggotaan->anggota->roleID == 4)  
                                        <span>{{ $keanggotaan->anggota->nama }} ({{ $keanggotaan->anggota->role->nama_role }})</span>
                                    @endif
                                    @endforeach

                                    {{-- Pengurus --}}
                                    @foreach ($jadwalmisa->kelompok->keanggotaans->where('periodeID', $jadwalmisa->periodeID) as $keanggotaan)
                                    @if ($keanggotaan->anggota->roleID == 3)  
                                        <span>{{ $keanggotaan->anggota->nama }} ({{ $keanggotaan->anggota->role->nama_role }})</span>
                                    @endif
                                    @endforeach
                                    ...
                                    {{-- Anggota --}}
                                    @foreach ($jadwalmisa->kelompok->keanggotaans->where('periodeID', $jadwalmisa->periodeID) as $keanggotaan)
                                    @if ($keanggotaan->anggota->roleID == 2)  
                                        <span>{{ $keanggotaan->anggota->nama }} ({{ $keanggotaan->anggota->role->nama_role }})</span>
                                    @endif
                                    @endforeach
                                </div>
                            </td>
                            <td class="px-4 py-3 border">{{ \Carbon\Carbon::parse($jadwalmisa->tanggal_jam_misa)->format('d M Y (H:i)') }}</td>
                            <td class="px-4 py-3 border">{{ $jadwalmisa->created_at->format('d M Y (H:i)') }}</td>
                            
                            <td class="px-4 py-3 flex items-center justify-center">
                                <button id="apple-imac-27-dropdown-button" data-dropdown-toggle="{{ $jadwalmisa->jadwalMisaID }}" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                    </svg>
                                </button>
                                <div id="{{ $jadwalmisa->jadwalMisaID }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm" aria-labelledby="apple-imac-27-dropdown-button">
                                        ...
                                        <li>
                                            <a href="/jadwalmisa/{{ $jadwalmisa->jadwalMisaID }}" >
