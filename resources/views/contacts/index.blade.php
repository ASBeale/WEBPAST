...
<body>
    ...
    <main class="p-4 md:ml-64 h-auto pt-7 bg-white">
        ...
        <div class="text-center mb-4">
            <h2 class="text-2xl font-semibold leading-7 text-gray-900">Contact</h2>
            <p class="mt-2 text-sm leading-6 text-gray-600">List Contact untuk website utama halaman Contact</p>
        </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-4 border">Title</th>
                            <th scope="col" class="px-4 py-4 border">Isi</th>
                            <th scope="col" class="px-4 py-4 border">Dibuat pada</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3 border">{{ $contact->title }}</td>
                            <td class="px-4 py-3 border">{{ $contact->isi }}</td>
                            <td class="px-4 py-3 border">{{ $contact->created_at->format('d M Y (H:i)') }}</td>
                            
                            <td class="px-4 py-3 flex items-center justify-center">
                                <button id="apple-imac-27-dropdown-button" data-dropdown-toggle="{{ $contact->contactID }}" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                    </svg>
                                </button>
                                <div id="{{ $contact->contactID }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                    <ul class="py-1 text-sm" aria-labelledby="apple-imac-27-dropdown-button">
                                        ...
                                        <li>
                                            <form action="{{ route('contacts.destroy', $contact->contactID) }}" method="post">
                                                @method('delete')
                                                @csrf
